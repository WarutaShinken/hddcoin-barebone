...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        ...
        if "hdd_target_address" not in self.config:
            self.config = config["farmer"]
        if "hdd_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "hdd_target_address" not in self.config or "hdd_target_address" not in self.pool_config:
