    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("hdd_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("hdd_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("hdd_target_address") or all_targets[-2] == config["pool"].get(
                    "hdd_target_address"
                ):
